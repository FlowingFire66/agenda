 ...
 * Description:
 */
public class DeleteParCommand implements CommandIn {
    ...
    public void simpleHelp() {
        ...
        System.out.println("删除一些参与人:deletePar -t 人大 -p ly2 ly3");
    }
    ...
    public boolean getOptions(String[] args) {
        ...
        try {
            ...
            hf.printHelp("deletePar", options, true);
            return false;
        }
    }
    ...
    public void excute() {
        ...
        if(Login.isLogin()) {
            ...
            if(agenda.getInitiatorName().equals(Login.getUser().getUsername())) {
                boolean flag = eleteUser(participators, agenda);
                if (flag) {
    ...
    private boolean eleteUser(String[] users, Agenda agenda) {
    ...
    public boolean checkParameters() {
        if (participators==null) return false;
        for (String s:
                participators) {
